...
public class DefaultEventSourceCollector implements EventSourceCollector {
	...
	private Set<EventSource> findEventSources(Object eventSourceProvider,
			EventSourceId providerId) {
		Class<?> eventSourceProviderClass = eventSourceProvider.getClass();
		EventSourceProviderClassInfo info = this.cache
				.get(eventSourceProviderClass);
		if (info == null) {
			info = new EventSourceProviderClassInfo(this.collectFieldInfos(
					eventSourceProviderClass, providerId),
					collectNestedEventSourceProviders(eventSourceProviderClass,
							providerId));
		...
		for (EventSourceFieldInfo fieldInfo : info.getFieldInfos()) {
			Object fieldValue = this.getFieldValue(fieldInfo.getField(),
					eventSourceProvider);
			if (fieldValue != null) {
				collectedSources.add(this.eventSourceFactory.createEventSource(
						fieldInfo.getEventSourceId(), fieldValue));
			}
		}
		for (EventSourceFieldInfo nestedProvider : info.getNestedProviders()) {
			Object fieldValue = this.getFieldValue(nestedProvider.getField(),
					eventSourceProvider);
			if (fieldValue != null) {
				// recursive call
				collectedSources.addAll(this.findEventSources(fieldValue,
						nestedProvider.getEventSourceId()));
			}
		}
	...
	/**
	 * Collects the nested event sources declared for a certain event source
	 * provider class.
	 * 
	 * @param eventSourceProviderClass
	 *            The class to check for event sources.
	 * @param providerId
	 *            The event source id of the given event source provider class
	 *            which will be the parent event source id of the event sources
	 *            found.
	 * @return A set of event sources encapsulated in an info object.
	 */
	private Set<EventSourceFieldInfo> collectFieldInfos(
			Class<?> eventSourceProviderClass, EventSourceId providerId) {
		...
		for (Field declaredField : declaredFields) {
			EventSourceId eventSourceFieldId = this.getEventSourceId(
					declaredField, providerId);
			if (eventSourceFieldId != null) {
				collectedFields.add(new EventSourceFieldInfo(declaredField,
						eventSourceFieldId));
			}
		}
	...
	/**
	 * Collects the nested event source providers declared for a certain event
	 ...
	 * @param providerId
	 *            The event source id of the given event source provider class
	 *            which will be the parent event source id of the nested event
	 *            source providers found.
	 * @return A set of nested event source providers encapsulated in an info
	 *         object.
	 */
	private Set<EventSourceFieldInfo> collectNestedEventSourceProviders(
			Class<?> eventSourceProviderClass, EventSourceId providerId) {
		Set<EventSourceFieldInfo> collectedFields = new HashSet<EventSourceFieldInfo>();
		Field[] declaredFields = eventSourceProviderClass.getDeclaredFields();
		for (Field declaredField : declaredFields) {
			EventSourceId eventSourceProviderId = this
					.getEventSourceProviderId(declaredField, providerId);
			if (eventSourceProviderId != null) {
				collectedFields.add(new EventSourceFieldInfo(declaredField,
						eventSourceProviderId));
			}
		}
		return collectedFields;
	}
	...
	private EventSourceId getEventSourceProviderId(Field declaredField,
			EventSourceId parentId) {
	...
	private EventSourceId getEventSourceId(Field declaredField,
			EventSourceId parentId) {
