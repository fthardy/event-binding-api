...
/**
 * This factory implementation creates instances
 * {@link GenericEventBindingConnector}.
 * 
 * @author Matthias Hanisch
 */
public class GenericEventBindingConnectorFactory implements
	...
	public EventBindingConnector createConnector(Object eventSource,
		...
		Method addMethod = findListenerMethod(eventSourceType, eventType,
				"add.+Listener");
		// find method for unregistering event listeners of the given type
		Method removeMethod = findListenerMethod(eventSourceType, eventType,
				"remove.+Listener");
		...
		if (addMethod != null && removeMethod != null) {
			...
			if (listenerTypeAdd.equals(listenerTypeRemove)) {
				...
				return new GenericEventBindingConnector(eventSource, addMethod,
						removeMethod, eventMethod, listenerTypeAdd);
			}
		}
	 ...
	 */
	private Method findListenerMethod(Class<?> eventSourceType,
			Class<?> eventType, String methodNameRegEx) {
		Filter<Method> filter = new Filter<Method>(new HashSet<Method>(
				Arrays.asList(eventSourceType.getMethods())))
				...
				.filter(new MethodNamePredicate(methodNameRegEx))
				.filter(new MethodParameterTypeHasEventMethodForTypePredicate(
						eventType));
	...
	private Method findEventMethod(Class<?> listenerType, Class<?> eventType) {
		Filter<Method> filter = new Filter<Method>(new HashSet<Method>(
				Arrays.asList(listenerType.getMethods())))
				.filter(new PublicMethodPredicate())
				.filter(new NotPredicate<Method>(new StaticMethodPredicate()))
				.filter(new MethodParameterCountPredicate(1))
