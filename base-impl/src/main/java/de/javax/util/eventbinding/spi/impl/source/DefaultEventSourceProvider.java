...
/**
 * This implementation of an event source provider is based on sources
 * which event source components are annotated with {@link de.javax.util.eventbinding.source.EventSource}
...
public class DefaultEventSourceProvider implements EventSourceProvider {
  ...
  private Set<EventSource> allEventSources;
	...
	private Set<EventSource> filterById(Set<EventSource> eventSources,
      String id) {
  ...
  @Override
	...
	private Set<EventSource> filterByType(Set<EventSource> eventSources,
      Class<?> eventType) {
    Set<EventSource> filteredSources = new HashSet<EventSource>();
    for(EventSource eventSource:eventSources) {
  ...
  private Set<EventSource> getEventSources() {
	  if(this.allEventSources==null) {
	    ...
	    this.allEventSources.addAll(findEventSources(this.source, this.source.getClass(), "", Collections.EMPTY_LIST));
	  }
  ...
  private boolean sourceSupportsEventType(Class<?> type, Class<?> eventType) {
    Filter<Method> filter = 
        new Filter<Method>(new HashSet<Method>(Arrays.asList(type.getMethods())))
        ...
        .filter(new MethodNamePredicate("add.+Listener"))
  ...
  private Set<EventSource> findEventSources(Object source, Class<?> sourceClass, String sourceId, List<Field> tree) {
    ...
    for(Field declaredField:declaredFields) {
      ...
      if(eventSourceFieldId!=null) {
        ...
        String id = sourceId.equals("")?eventSourceFieldId:sourceId+"."+eventSourceFieldId;
      ...
      if(eventSourceProviderPrefix!=null) {
        String prefix=sourceId.equals("")?eventSourceProviderPrefix:sourceId+"."+eventSourceProviderPrefix;
  ...
  private String getEventSourceProviderPrefix(Field declaredField) {
    de.javax.util.eventbinding.source.EventSourceProvider annotation = declaredField.getAnnotation(de.javax.util.eventbinding.source.EventSourceProvider.class);
    if(annotation==null) {
      return null;
    } else {
  ...
  private EventSource createEventSource(Object source,
      String eventSourceId, String alias, List<Field> tree, Field declaredField) {
    List<Field> fields = new ArrayList<Field>(tree);
    fields.add(declaredField);
  ...
  private static String getEventSourceId(Field declaredField) {
    de.javax.util.eventbinding.source.EventSource annotation = declaredField.getAnnotation(de.javax.util.eventbinding.source.EventSource.class);
    if(annotation==null) {
      return null;
    } else {
      return annotation.value();
