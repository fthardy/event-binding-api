...
public class DefaultEventTargetCollector implements EventTargetCollector {
     ...
     *            the ID-selector expression prefix from the parent event target provider instance, if
     *            <code>eventTargetProvider</code> is a nested event target provider. Otherwise this parameter is
     *            <code>null</code> (only when called by {@link #collectEventTargetsFrom(Object)}).
     * 
    ...
    protected Set<EventTarget> collectEventTargets(Object eventTargetProvider, String idSelectorPrefix) {
        ...
        if (eventTargetProvider != null) {
            TargetProviderDescriptor targetProviderDescriptor = this.targetProviderClassAnalyzer
                    .getDescriptorFor(eventTargetProvider.getClass());
    ...
    private Object getNestedTargetProviderFromField(Field field, Object fieldOwner) {
        ...
        try {
            ...
            throw new EventTargetAccessException("Failed to access field '" + field.toGenericString() + "' in class '"
                    + fieldOwner.getClass() + "'!", e);
        } finally {
