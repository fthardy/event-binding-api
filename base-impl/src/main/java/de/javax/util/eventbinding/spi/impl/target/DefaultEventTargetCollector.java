...
public class DefaultEventTargetCollector implements EventTargetCollector {
                ...
                try {
                    ...
                    if (nestedTargetProvider == null) {
                        throw new EventTargetAccessException("The value of field '"
                                + nestedProviderField.toGenericString() + "' in class '" + targetProviderClass
                                + "' is null!");
                    }
                } finally {
    ...
    private Set<HandlerMethodInfo> collectHandlerMethodInfos(Class<?> targetProviderClass) {
        Set<HandlerMethodInfo> infos = new HashSet<HandlerMethodInfo>();
        for (Method method : targetProviderClass.getMethods()) {
            int modifiers = method.getModifiers();
            boolean isAccepted = Modifier.isPublic(modifiers) && !Modifier.isAbstract(modifiers)
            ...
            if (isAccepted) {
                ...
                for (Annotation annotation : parameterAnnotations) {
                    if (annotation.annotationType() == HandleEvent.class) {
                        String selectorExpression = ((HandleEvent) annotation).from().trim();
                        if (selectorExpression.isEmpty()) {
                            selectorExpression = EventSourceIdSelector.WILDCARD;
                        }
                        infos.add(new HandlerMethodInfo(method, this.idSelectorFactory
    ...
    private Set<NestedProviderFieldInfo> collectionNestedTargetProviderFieldInfos(Class<?> targetProviderClass) {
        ...
        for (Field field : targetProviderClass.getDeclaredFields()) {
            ...
            if (annotation != null) {
                ...
                infos.add(new NestedProviderFieldInfo(field, this.idSelectorFactory
                        .createEventSourceIdSelector(selectorExpression)));
            }
        }
        return infos;
