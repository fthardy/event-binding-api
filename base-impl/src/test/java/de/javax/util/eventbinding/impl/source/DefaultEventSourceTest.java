...
public class DefaultEventSourceTest {
	...
	public void prepare() throws Exception {
		...
		contactEditorGui = new ContactEditorGui(personEditorGui,
				adressEditorGui);
		...
		collectedEventSources = eventSourceCollector
				.collectEventSourcesFrom(contactEditorGui);
	}

	@Test
	public void bindToTarget() throws Exception {
		checkBindToTarget("personEditor.firstNameField", TextChangeEvent.class,
				true);
		checkBindToTarget("personEditor.lastNameField", TextChangeEvent.class,
				true);
		checkBindToTarget("personEditor.birthDateField",
				CalendarChangeEvent.class, true);

		checkBindToTarget("personEditor.birthDateField", TextChangeEvent.class,
				false);

		checkBindToTarget("addressEditor.streetField", TextChangeEvent.class,
				true);
		checkBindToTarget("addressEditor.zipField", TextChangeEvent.class, true);
		checkBindToTarget("addressEditor.cityField", TextChangeEvent.class,
				true);
	...
	public void bindToTargetWildcard() throws Exception {
		checkBindToTarget("personEditor.firstNameField", "personEditor.*",
				TextChangeEvent.class, true);
		checkBindToTarget("personEditor.lastNameField", "personEditor.*",
				TextChangeEvent.class, true);
		checkBindToTarget("personEditor.birthDateField", "personEditor.*",
				CalendarChangeEvent.class, true);
		checkBindToTarget("personEditor.firstNameField", "*",
				TextChangeEvent.class, true);
		checkBindToTarget("personEditor.lastNameField", "*",
				TextChangeEvent.class, true);
		checkBindToTarget("personEditor.birthDateField", "*",
				CalendarChangeEvent.class, true);

		checkBindToTarget("personEditor.birthDateField", "personEditor.*",
				TextChangeEvent.class, false);
		checkBindToTarget("personEditor.birthDateField", "*",
				TextChangeEvent.class, false);

		checkBindToTarget("addressEditor.streetField", "addressEditor.*",
				TextChangeEvent.class, true);
		checkBindToTarget("addressEditor.zipField", "addressEditor.*",
				TextChangeEvent.class, true);
		checkBindToTarget("addressEditor.cityField", "addressEditor.*",
				TextChangeEvent.class, true);
		checkBindToTarget("addressEditor.streetField", "*",
				TextChangeEvent.class, true);
		checkBindToTarget("addressEditor.zipField", "*", TextChangeEvent.class,
				true);
		checkBindToTarget("addressEditor.cityField", "*",
				TextChangeEvent.class, true);

		checkBindToTarget("okButton", "*", ButtonClickEvent.class, true);
		checkBindToTarget("cancelButton", "*", ButtonClickEvent.class, true);
		checkBindToTarget("okButton", "personEditor.*", ButtonClickEvent.class,
				false);
		checkBindToTarget("cancelButton", "personEditor.*",
				ButtonClickEvent.class, false);
	}

	private void checkBindToTarget(String eventSourceIdSelectorExpression,
			Class<?> eventType, boolean expectedToBind) throws Exception {
		checkBindToTarget(eventSourceIdSelectorExpression,
				eventSourceIdSelectorExpression, eventType, expectedToBind);
	}

	private void checkBindToTarget(String eventSourceIdSelectorExpression,
			String eventTargetIdExpression, Class<?> eventType,
			boolean expectedToBind) throws Exception {
		EventSourceIdSelector eventSourceIdSelector = new DefaultEventSourceIdSelector(
				eventSourceIdSelectorExpression);
		EventSourceIdSelector eventTargetIdSelector = new DefaultEventSourceIdSelector(
				eventTargetIdExpression);
		EventSource eventSource = getEventSource(collectedEventSources,
				eventSourceIdSelector);
		Assert.assertNotNull(eventSource);
		EventTarget eventTarget = new DefaultEventTarget(eventTargetIdSelector,
